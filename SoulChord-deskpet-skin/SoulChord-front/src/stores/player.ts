...
export const usePlayerStore = defineStore('player', () => {
  ...
  function initAudio(audio: HTMLAudioElement) {
    ...
    // ended 由 useAudio.ts 中的独立 listener 处理，此处只发 play_end 反馈
  ...
  /**
   * 切歌（用户点击下一首 或 歌曲自然结束）
   * @param fromEnded 是否由歌曲 ended 事件触发（自然结束 → 靠后端 play_end 驱动，不重复发 skip）
   */
  function next(fromEnded = false) {
    if (!currentSong.value) return
    // ★ 队列空时：自然结束靠后端 play_end 事件，用户主动点击则发 skip 通知后端
  ...
  return {
    ...
    setPlaybackMode, addToQueue, removeFromQueue, clearQueue, toggleLike, dislike, restoreFromCache,
  }
})
