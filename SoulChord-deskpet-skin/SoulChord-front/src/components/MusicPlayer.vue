...
/** 拖动时的视觉进度（0-100），脱离 timeupdate 依赖，保证跟手 */
const dragPreview = ref(0)
...
function handleProgressClick(e: MouseEvent) {
  if (isDragging.value) return  // 拖拽结束时 click 也会触发，跳过重复 seek
...
function handleProgressMouseDown(e: MouseEvent) {
  ...
  function updatePosition(ev: MouseEvent) {
    ...
    // ★ 仅更新视觉反馈，不 seek 音频 — 避免频繁 seek 导致音频重载
    dragPreview.value = ratio * 100
  }

  function onMove(ev: MouseEvent) { updatePosition(ev) }

  function onUp(ev: MouseEvent) {
    document.removeEventListener('mousemove', onMove)
    document.removeEventListener('mouseup', onUp)
    // ★ 释放时才 seek 一次，避免拖动中反复中断缓冲
    const rect = bar.getBoundingClientRect()
    const ratio = Math.max(0, Math.min(1, (ev.clientX - rect.left) / rect.width))
    playerStore.seekTo(ratio * playerStore.duration)
    // 延迟重置 isDragging，让 click 事件在 isDragging=true 时被跳过
  ...
  <div class="music-player">
    ...
    <div class="music-player__progress">
      ...
      <div
        ...
        :class="{ 'music-player__progress-bar--dragging': isDragging }"
      ...
      >
        ...
        <div
          ...
          :style="{ width: (isDragging ? dragPreview : playerStore.progress) + '%' }"
        ></div>
        <div
          class="music-player__progress-thumb"
          :class="{ 'music-player__progress-thumb--dragging': isDragging }"
          :style="{ left: (isDragging ? dragPreview : playerStore.progress) + '%' }"
        ></div>
      </div>
  ...
  &__progress-bar {
    ...
    &--dragging {
      ...
      .music-player__progress-thumb {
        transition: none;  /* 拖动时禁用过渡动画，保证视觉跟手 */
      }
    }
  }
