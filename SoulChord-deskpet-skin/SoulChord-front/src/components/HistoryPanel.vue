    ...
    <div v-if="visible" class="history-panel">
      ...
      <div v-else class="history-panel__list">
        <div v-for="item in items" :key="item.song_id + item.played_at" class="history-panel__item">
          ...
          <span class="history-panel__item-song">{{ item.song_name }}<span v-if="item.artist" class="history-panel__item-artist"> — {{ item.artist }}</span></span>
...
.history-panel {
  ...
  &__item-artist { color: $text-muted; font-size: 10px; }
