...
### 2.2.2 chat.voice_text

麦克风语音经ASR识别后的文本传输，不传输音频二进制流
...
### 2.3.6 music.tts_speak（★ v0.6.1 E 项新增：song_finished queue 空生成过渡语）

当播放队列自然耗尽时（song_finished 且 playlist_queue 为空），后端自动生成过渡语走前端 TTS 播报，让用户知道"在找新歌"。
...
字段释义：
1. `text`：过渡语文本（**规则生成**，按 `program_mood` + `today_theme` 出 4 档模板：energetic / warm / reflective / fallback）
2. `voice`：TTS 音色（默认 `warm`；MVP 不接真实 TTS 时前端用本地 toast 兜底）
3. `source`：触发源（v0.6.1 唯一来源 `song_finished_queue_empty`）
4. `mood`：当前 program_mood（energetic / warm / reflective / neutral）
5. `theme`：当前 today_theme（来自 `data/program_state.json`）

**前端实现**：
- 监听 `music.tts_speak` → 调 TTS 播报（或本地 toast 兜底）
- 监听 `status.transition_speech` → 可选：状态栏展示"在找新歌"提示
