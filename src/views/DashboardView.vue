...
watch(() => chatStore.currentStreamingMessage, () => scrollToBottom())
...
<template>
  <div class="dashboard">
    <!-- 播放器区域 -->
    <section class="dashboard__player">
      <MusicPlayer />
    </section>

    <!-- 分隔线 -->
    <div class="dashboard__divider">
      <div class="dashboard__divider-line"></div>
      <span class="dashboard__divider-text">AI DJ</span>
      <div class="dashboard__divider-line"></div>
    </div>
    ...
    <section class="dashboard__chat">
      ...
      <div ref="messageListRef" class="dashboard__messages">
        ...
        <div v-if="chatStore.isStreaming" class="dashboard__streaming">
          <ChatBubble
            :message="{
              id: 'streaming',
              role: 'assistant',
      ...
      <div class="dashboard__scenes">
        <button
          ...
          :key="scene.id"
          class="dashboard__scene-btn"
          :disabled="isSending"
          @click="handleScene(scene.id)"
        >
          {{ scene.id === 'tired' ? '🌙' : scene.id === 'focus' ? '🎯' : scene.id === 'happy' ? '☀️' : scene.id === 'sleep' ? '💤' : scene.id === 'commute' ? '🚗' : scene.id === 'workout' ? '🏃' : '💬' }}
          {{ scene.label }}
        </button>
      </div>
